...
public class UsersRepositoryJdbcImpl implements UsersRepository {
    private static String QUERY_STATEMENT = "SELECT (SELECT COUNT(id) FROM chat.users) as sum, users.id as user_id, users.login as user_login, users.password as user_pas, chatrooms.id as room_id, chatrooms.name as room_name, chatrooms.owner as room_owner, a1.id as room_user_id, a1.name as room_user_name, a1.owner as room_user_owner\n" +
            "    FROM chat.users\n" +
            "             LEFT JOIN chat.chatrooms ON users.id = chatrooms.owner\n" +
            "             LEFT JOIN chat.messages ON users.id = messages.author\n" +
            "             LEFT JOIN chat.chatrooms a1 ON messages.room = a1.id\n" +
            "    ORDER BY users.id, chatrooms.id";
    ...
    public List<User> findAll(int page, int size) {
        ...
        try {
            ...
            long id = 1, prev_id = -1, current_id;
            resultSet.next();
            ...
            while (id < pages) {
                ...
                if (list.get(user_id) != null)
                {
                    ...
                    for (Chatroom room : list.get(user_id).getChatroomList())
                    {
                    ...
                    if (!finded) {
                        list.get(user_id).getChatroomList().add(createChatroom(resultSet, 1));
                    }
                    finded = false;
                    chatroom_id = resultSet.getLong("room_user_id");
                    for (Chatroom room : list.get(user_id).getActiveChatroomList())
                    {
                        if (room.getId() == chatroom_id) {
                            finded = true;
                            break;
                    ...
                    if (!finded) {
                        list.get(user_id).getActiveChatroomList().add(createChatroom(resultSet, 2));
                    }
                }
                else {
